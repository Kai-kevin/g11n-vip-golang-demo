...
func TestGetTranslationByKey(t *testing.T) {
	for _, local := range Locales {
		for _, component := range Components {
			t.Logf("Request parameters:key:\"%v\",local:\"%v\",component:\"%v\"", Key, local, component)
			value, err := getTranslationByKey(Key, local, component)
			t.Logf("Result:\"%v\" convert to \"%v\"", Key, value)
			if err != nil {
...
func TestGetTranslationByKeyWithParams(t *testing.T) {
	for _, local := range Locales {
		for _, component := range Components {
			t.Logf("Request parameters:key:\"%v\",local:\"%v\",component:\"%v\",param:\"%v\"", Key, local, component, param)
